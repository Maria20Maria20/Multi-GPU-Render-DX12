...
void HybridSSAOApp::PopulateAmbientMapCommands(const std::shared_ptr<GCommandList>& cmdList) const
{
		...
		if (IsUseHBAO)
		{
			...
			}
			{
					...
					currentFrameResource->SecondRenderFenceValue))
				{
					...
					secondCmdList->CopyResource(CrossResource.GetAmbientMap().GetSharedResource(),
						Resources.GetAmbientMap());
		...
		else if (IsUseXeGTAO)
		{
			...
			}
			{
					...
					currentFrameResource->SecondRenderFenceValue))
				{
					...
					xegtaoPass->Compute(secondCmdList, currentFrameResource->SecondXeGTAOConstantUploadBuffer,
						Resources);
		...
		else
		{
			...
			}
			{
					...
					currentFrameResource->SecondRenderFenceValue))
				{
					...
					ssaoPass->ComputeSsao(secondCmdList, currentFrameResource->SecondSsaoConstantUploadBuffer,
						Resources, 1);
	...
	{
		if (IsUseHBAO)
			hbaoPass->Compute(cmdList, currentFrameResource->PrimeHBAOConstantUploadBuffer,
				hbaoPass->GetPrimeResources());
		else if (IsUseXeGTAO)
			xegtaoPass->Compute(cmdList, currentFrameResource->PrimeXeGTAOConstantUploadBuffer,
				xegtaoPass->GetPrimeResources());
		else
			ssaoPass->ComputeSsao(cmdList, currentFrameResource->PrimeSsaoConstantUploadBuffer,
				ssaoPass->GetPrimeResources(), 3);
	}
}
...
void HybridSSAOApp::PopulateForwardPathCommands(const std::shared_ptr<GCommandList>& cmdList)
{
	//Forward Path with SSAA
	{
		...
		if (IsUseHBAO)
			...
			cmdList->SetRootDescriptorTable(StandardShaderSlot::AmbientMap,
				xegtaoPass->GetPrimeResources().GetAmbientMapSRV());
		else
			cmdList->SetRootDescriptorTable(StandardShaderSlot::AmbientMap,
				ssaoPass->GetPrimeResources().GetAmbientMapSRV(), 0);
...
void HybridSSAOApp::PopulateDrawFullQuadTexture(const std::shared_ptr<GCommandList>& cmdList,
	...
	const UINT renderTextureMemoryOffset,
	const GraphicPSO& pso) const
{
	cmdList->SetPipelineState(pso);

	cmdList->SetDescriptorsHeap(renderTextureSRVMemory);
	cmdList->SetGraphicsRootDescriptorTable(StandardShaderSlot::AmbientMap, renderTextureSRVMemory,
		renderTextureMemoryOffset);
		...
		for (int i = 1; i <= maxLevel; ++i)
		{
			auto& NativeSSAOState = benchmark.AddState<WaitState>(
				TestTime, FileQueueWriter(Benchmark::GetLogFile(std::wstring(L"SSAA-") + std::to_wstring(i) +
					L" Native SSAO ", *primeDevice, *secondDevice)));
			NativeSSAOState.OnEnter = [this, i](FileQueueWriter& logs)
				{
					Flush();
					IsUsingSharedSSAO = false;
					IsUseHBAO = false;
					IsUseXeGTAO = false;
					antiAliasingPrimePath->SetMultiplier(i, MainWindow->GetClientWidth(), MainWindow->GetClientHeight());
					logs.PushMessage(L"FPS;MSPF;MinFPS;MinMSPF;MaxFPS;MaxMSPF");
				};

			NativeSSAOState.OnStatChanged = [this, i](FileQueueWriter& logs, const TimeStats& ts, float progress)
				{
					Benchmark::PrintStatsCSV(ts, logs);
					MainWindow->SetWindowTitle(
						L"Native SSAO Progress " + std::format(L"{:.2f}", progress * 100) + L"% FPS:" + std::to_wstring(ts.fps)
						+ L" SSAA:" + std::to_wstring(i));
				};
			...
			auto& HybridSSAOState = benchmark.AddState<WaitState>(
				TestTime, FileQueueWriter(Benchmark::GetLogFile(std::wstring(L"SSAA-") + std::to_wstring(i)
					+ L" Hybrid SSAO ", *primeDevice, *secondDevice)));
			HybridSSAOState.OnEnter = [this, i](FileQueueWriter& logs)
				{
					ResetCamera();
					Flush();
					antiAliasingPrimePath->SetMultiplier(i, MainWindow->GetClientWidth(), MainWindow->GetClientHeight());
			...
			HybridSSAOState.OnStatChanged = [this, i](FileQueueWriter& logs, const TimeStats& ts, float progress)
				{
					Benchmark::PrintStatsCSV(ts, logs);
					MainWindow->SetWindowTitle(
						L"Hybrid SSAO Progress " + std::format(L"{:.2f}", progress * 100) + L"% FPS:" + std::to_wstring(ts.fps)
						+ L" SSAA:" + std::to_wstring(i));
				};
			...
			auto& NativeHBAOState = benchmark.AddState<WaitState>(
				TestTime, FileQueueWriter(Benchmark::GetLogFile(std::wstring(L"SSAA-") + std::to_wstring(i) +
					L" Native HBAO ", *primeDevice, *secondDevice)));
			NativeHBAOState.OnEnter = [this, i](FileQueueWriter& logs)
				{
					ResetCamera();
					Flush();
					antiAliasingPrimePath->SetMultiplier(i, MainWindow->GetClientWidth(), MainWindow->GetClientHeight());
			...
			NativeHBAOState.OnStatChanged = [this, i](FileQueueWriter& logs, const TimeStats& ts, float progress)
				{
					Benchmark::PrintStatsCSV(ts, logs);
					MainWindow->SetWindowTitle(
						L"Native HBAO Progress " + std::format(L"{:.2f}", progress * 100) + L"% FPS:" + std::to_wstring(ts.fps)
						+ L" SSAA:" + std::to_wstring(i));
				};
			...
			auto& HybridHBAOState = benchmark.AddState<WaitState>(
				TestTime, FileQueueWriter(Benchmark::GetLogFile(std::wstring(L"SSAA-") + std::to_wstring(i)
					+ L" Hybrid HBAO ", *primeDevice, *secondDevice)));
			HybridHBAOState.OnEnter = [this, i](FileQueueWriter& logs)
				{
					antiAliasingPrimePath->SetMultiplier(i, MainWindow->GetClientWidth(), MainWindow->GetClientHeight());
					ResetCamera();
					SwitchDevice();
			...
			HybridHBAOState.OnStatChanged = [this, i](FileQueueWriter& logs, const TimeStats& ts, float progress)
				{
					Benchmark::PrintStatsCSV(ts, logs);
					MainWindow->SetWindowTitle(
			...
			auto& NativeXeGTAOState = benchmark.AddState<WaitState>(
				TestTime, FileQueueWriter(Benchmark::GetLogFile(std::wstring(L"SSAA-") + std::to_wstring(i) +
					L" Native XeGTAO ", *primeDevice, *secondDevice)));
			NativeXeGTAOState.OnEnter = [this, i](FileQueueWriter& logs)
				{
					ResetCamera();
					Flush();
					antiAliasingPrimePath->SetMultiplier(i, MainWindow->GetClientWidth(), MainWindow->GetClientHeight());
			...
			NativeXeGTAOState.OnStatChanged = [this, i](FileQueueWriter& logs, const TimeStats& ts, float progress)
				{
					Benchmark::PrintStatsCSV(ts, logs);
					MainWindow->SetWindowTitle(
			...
			auto& HybridXeGTAOState = benchmark.AddState<WaitState>(
				TestTime, FileQueueWriter(Benchmark::GetLogFile(std::wstring(L"SSAA-") + std::to_wstring(i)
					+ L" Hybrid XeGTAO ", *primeDevice, *secondDevice)));
			HybridXeGTAOState.OnEnter = [this, i](FileQueueWriter& logs)
				{
					ResetCamera();
					Flush();
					antiAliasingPrimePath->SetMultiplier(i, MainWindow->GetClientWidth(), MainWindow->GetClientHeight());
			...
			HybridXeGTAOState.OnStatChanged = [this, i](FileQueueWriter& logs, const TimeStats& ts, float progress)
				{
					Benchmark::PrintStatsCSV(ts, logs);
					MainWindow->SetWindowTitle(
			...
			HybridXeGTAOState.OnExit = [this, i](FileQueueWriter& logs)
				{
					logs.WriteAllLog();
					SwitchDevice();
					Flush();
...
void HybridSSAOApp::InitRenderPaths()
{
	...
	hbaoPass->Initialize(primeDevice, secondDevice, layoutDesc, MainWindow->GetClientWidth(),
		MainWindow->GetClientHeight());
	hbaoPass->OnResize(MainWindow->GetClientWidth(), MainWindow->GetClientHeight());

	xegtaoPass->Initialize(primeDevice, secondDevice, layoutDesc, MainWindow->GetClientWidth(),
		MainWindow->GetClientHeight());
