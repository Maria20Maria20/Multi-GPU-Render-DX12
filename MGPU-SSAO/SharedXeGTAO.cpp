	...
	const std::shared_ptr<ConstantUploadBuffer<GTAOConstants>>& Constants) const
{
	...
	if (pass.srvCount > 0)
	{
		cmdList->SetComputeRootDescriptorTable(paramIndex, Resources.GetDepthMapSRV());
		paramIndex++;
	}

	if (pass.uavCount > 0)
	{
		cmdList->SetComputeRootDescriptorTable(paramIndex, Resources.GetAmbientMapUAV());
		paramIndex++;
	}

	//if (pass.extraCbvCount > 0)
	//{
	//	cmdList->SetComputeRootConstantBufferView(paramIndex, *Resources.compositeCB);
	//}
