...
void CrossAdapterParticleEmitter::InitPSO(const std::shared_ptr<GDevice>& otherDevice)
{
...
void CrossAdapterParticleEmitter::Dispatch(const std::shared_ptr<GCommandList>& cmdList)
{
    ...
    if (DirtyActivated == Disable)
    {
        ...
        cmdList->CopyResource(primeParticleEmitter->ParticlesAlive->GetD3D12Resource(),
                              CrossAdapterAliveIndexes->GetPrimeResource().GetD3D12Resource());
        cmdList->CopyResource(primeParticleEmitter->ParticlesDead->GetD3D12Resource(),
    ...
    if (UseSharedCompute)
    {
                                                                                          ...
                                                                                          ParticlesAliveCount)
        {
            ...
            if (check >= primeParticleEmitter->emitterData.ParticleInjectCount)
            {
                ...
                cmdList->SetRoot32BitConstants(ParticleComputeSlot::EmitterData, sizeof(EmitterData) / sizeof(float),
                                               &primeParticleEmitter->emitterData, 0);

                cmdList->Dispatch(primeParticleEmitter->emitterData.InjectedGroupCount,
                                  primeParticleEmitter->emitterData.InjectedGroupCount, 1);
        ...
        {
            primeParticleEmitter->emitterData.SimulatedGroupCount = primeParticleEmitter->CalculateGroupCount(
                primeParticleEmitter->emitterData.ParticlesAliveCount);
            ...
            cmdList->Dispatch(primeParticleEmitter->emitterData.SimulatedGroupCount,
                              primeParticleEmitter->emitterData.SimulatedGroupCount, 1);
        }
