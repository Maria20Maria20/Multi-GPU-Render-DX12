...
{
    auto firstDevice = GDeviceFactory::GetAllDevices(true)[GraphicAdapterPrimary];
    auto otherDevice = GDeviceFactory::GetAllDevices(true)[GraphicAdapterSecond];
...
{
    const auto commandQueue = secondDevice->GetCommandQueue(GQueueType::Graphics);
...
void SampleApp::Draw(const GameTimer& gt)
{
    ...
    auto primeDeviceCopyQueue = primeDevice->GetCommandQueue(GQueueType::Copy);
    ...
    cmdList->FlushResourceBarriers();
    cmdList->CopyTextureRegion(crossAdapterBackBuffers[currentBackBufferIndex]->GetPrimeResource(), 0, 0, 0,
                               primeDeviceBackBuffers[currentBackBufferIndex], &copyRegionBox);
    cmdList->TransitionBarrier(crossAdapterBackBuffers[currentBackBufferIndex]->GetPrimeResource(),
                               D3D12_RESOURCE_STATE_COMMON);
    cmdList->TransitionBarrier(primeDeviceBackBuffers[currentBackBufferIndex], D3D12_RESOURCE_STATE_COMMON);
    cmdList->FlushResourceBarriers();
    ...
    cmdList->FlushResourceBarriers();
    cmdList->CopyTextureRegion(MainWindow->GetCurrentBackBuffer(), 0, 0, 0,
                               crossAdapterBackBuffers[currentBackBufferIndex]->GetSharedResource(), &copyRegionBox);
...
void SampleApp::OnResize()
{
    ...
    secondRect = D3D12_RECT{
        MainWindow->GetClientWidth() / 2, 0, MainWindow->GetClientWidth(), MainWindow->GetClientHeight()
    };
}

bool SampleApp::InitMainWindow()
{
